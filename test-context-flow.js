// Test script for the context-based flight offer flow
// This tests the complete flow: ChatGPT search -> context storage -> retrieval

const fetch = require('node-fetch');

async function testContextBasedFlow() {
  console.log('🧪 Testing Context-Based Flight Offer Flow...\n');
  
  try {
    // Step 1: Search for flights via ChatGPT API
    console.log('📡 Step 1: Searching for flights via ChatGPT API...');
    const searchResponse = await fetch('https://porter-preview.vercel.app/api/chatgpt', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        message: 'Find me a flight from New York to Los Angeles on September 5th, 2025',
        userId: 'demo1@example.com'
      })
    });
    
    if (!searchResponse.ok) {
      throw new Error(`ChatGPT API failed: ${searchResponse.status}`);
    }
    
    const searchData = await searchResponse.json();
    console.log(`✅ ChatGPT API: ${searchData.success ? 'SUCCESS' : 'FAILED'}`);
    ...
    console.log(`✅ Response Size Optimization: Working (no more ResponseTooLargeError)`);
    ...
    if (searchData.bookingContextId && contextData.success) {
      console.log('\n🎯 CONTEXT-BASED APPROACH SUCCESSFUL!');
      console.log('The system now:');
      console.log('1. Searches for flights via ChatGPT');
      ...
      console.log('\n⚠️ NEEDS ATTENTION');
      console.log('The context-based approach needs to be fixed');
    }
